angular.module('pokedex').service('regionService', function($http) {
...
// var getKanto = function() {
//   return $http({
//     method: 'GET',
//     url: './js/json/kanto.json'
//   });
// };
...
var kalosMtn = [
  ...
  this.getID = function(pokemon) {
    if (isNaN(pokemon)) {
      var id;
      function idGetter(pokemon, arr) {
        for (var i = 0; i < arr.length; i++) {
          if (pokemon === arr[i].pokemon_species.name) {
            id = arr[i].entry_number;
          }
        }
      }
      idGetter(pokemon, national);
      return id;
    }
    else {
...
function internalID(num, arr) {
  for (var i = 0; i < arr.length; i++) {
    if (Number(num) === arr[i].entry_number) {
      return arr[i].pokemon_species.name;
    }
  }
}
  ...
  this.getRegions = function(pokemon) {
    ...
    function regionLooperName(pokemon, arr) {
      for (var i = 0; i < arr.length - 1; i++) {
        if(pokemon === arr[i].pokemon_species.name){
          areas.push(arr[arr.length - 1].region);
        }
      }
    }
    function regionLooperNum(pokemon, arr, cb) {
      var abcID = cb(pokemon, national);
      for (var i = 0; i < arr.length - 1; i++) {
        if(abcID === arr[i].pokemon_species.name) {
          areas.push(arr[arr.length - 1].region);
        }
      }
    }
    if (isNaN(pokemon)) {
      ...
      regionLooperName(pokemon, kanto);
      regionLooperName(pokemon, johto);
      regionLooperName(pokemon, hoenn);
      regionLooperName(pokemon, sinnoh);
      regionLooperName(pokemon, unova);
      regionLooperName(pokemon, kalosCentral);
      regionLooperName(pokemon, kalosCoastal);
      regionLooperName(pokemon, kalosMtn);
    }
    else {
      regionLooperNum(pokemon, kanto, internalID);
      regionLooperNum(pokemon, johto, internalID);
      regionLooperNum(pokemon, hoenn, internalID);
      regionLooperNum(pokemon, sinnoh, internalID);
      regionLooperNum(pokemon, unova, internalID);
      regionLooperNum(pokemon, kalosCentral, internalID);
      regionLooperNum(pokemon, kalosCoastal, internalID);
      regionLooperNum(pokemon, kalosMtn, internalID);
    }
    if (!areas[0]) {
      areas.push('National');
      return areas;
